...
<Callout type="info">
  **Try it yourself.** Give your agent this prompt:
  ...
  Bridge 100 USDC from Arbitrum to Base
  ```
  With the [Across skill installed](/ai-agents), the agent handles the entire flow autonomously.
</Callout>
...
<Callout type="info">
  **Try it yourself.** Give your agent this prompt:
  ...
  Bridge 100 USDC from Arbitrum to Ethereum, swap to ETH, and deposit into Aave
  ```
  This uses [embedded crosschain actions](/introduction/embedded-actions) to compose bridge + DeFi in a single user transaction.
</Callout>
...
<Cards>
  <Card title="Integrate Swap API" href="/guides/dev-guides/integrate-swap-api">
    Full developer guide for the 5-step bridge flow used in Example 1.
  </Card>
  <Card title="Crosschain Aave Deposit" href="/guides/dev-guides/crosschain-aave-deposit">
