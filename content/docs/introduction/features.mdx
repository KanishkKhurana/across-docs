...
Across is unique in offering three independent settlement pathways. The [Swap API](/introduction/swap-api) automatically selects the optimal mechanism for each transfer.

<Cards>
  <Card title="Intents — Relayer Network" href="/guides/concepts/intent-lifecycle">
  ...
  <Card title="CCTP V2 / CCTPFast — Native USDC" href="/introduction/stablecoins">
  ...
  <Card title="OFT — Native USDT0" href="/introduction/stablecoins">
...
Across's relayer network fills most transfers in **~2 seconds** on mainnet. Relayers compete on speed, fronting their own capital on the destination chain and getting repaid later through the [settlement process](/guides/concepts/intent-lifecycle).
...
See the full list via the [`/swap/chains`](/api-reference) endpoint.
...
Bridge and execute in one transaction. Attach arbitrary contract calls to your crosschain transfer — the [MulticallHandler](/introduction/embedded-actions) executes them atomically on the destination chain.
...
<Callout type="info">
  V4 is rolling out progressively. See [Across V4](/guides/concepts/across-v4) for technical details.
</Callout>
