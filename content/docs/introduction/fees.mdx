...
<Callout type="info">
  **When are LP fees zero?** If the relayer takes repayment on the origin chain (same chain as the deposit), no cross-chain rebalancing is needed, so LP fees are zero. Non-zero fees apply when repayment occurs on a different chain.
</Callout>
