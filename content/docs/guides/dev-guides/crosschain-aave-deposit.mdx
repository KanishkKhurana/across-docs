...
- Basic familiarity with [the Swap API](/introduction/swap-api) — if you haven't read the [basic integration guide](/guides/dev-guides/integrate-swap-api), start there
...
- **Use dynamic token amounts** — set `populateDynamically: true` with `balanceSourceToken` to forward the exact swapped balance for ERC-20 actions. See [Transfer ERC-20 Tokens](/introduction/embedded-actions/transfer-erc20).
- **Chain multiple actions** — add more objects to the `actions` array to execute a sequence of operations on the destination.
- **Track the deposit** — poll `/deposit/status` to confirm everything landed. See [Tracking Deposits](/introduction/tracking-deposits).
