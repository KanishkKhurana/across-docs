  ...
  const handleCheck = useCallback(async () => {
    ...
    try {
      ...
      if (matches.length === 0) {
        ...
        // Use first match as representative token info
    ...
    <div className="mt-6 space-y-4">
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Enter token symbol or address (e.g. USDC, 0xaf88...)"
            className={cn(
        ...
        <div className="rounded-xl border border-[var(--color-fd-border)] bg-[var(--color-fd-card)] overflow-hidden">
          <div className="border-l-4 border-l-[var(--color-fd-primary)] p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-[var(--color-fd-muted-foreground)]">
                {result.token.decimals} decimals
              </span>
            </div>
            <p className="text-sm text-[var(--color-fd-muted-foreground)] mb-4">
              Supported on {result.chains.length} chain{result.chains.length !== 1 ? 's' : ''}:
            </p>
                ...
                <tbody>
                  {result.chains.map(({ chain, address }) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-4 text-[var(--color-fd-muted-foreground)]">
                        {chain.chainId}
                      </td>
                      <td className="py-2 font-mono text-xs text-[var(--color-fd-muted-foreground)]">
                        {address}
                      </td>
                    </tr>
                  ))}
      ...
      {notFound && (
        <div className="rounded-xl border border-[var(--color-fd-border)] bg-[var(--color-fd-card)] p-5">
          ...
          <p className="text-sm text-[var(--color-fd-muted-foreground)] mb-4">
            &quot;{query.trim()}&quot; is not currently supported by Across. If you&apos;d like to request support for this token, reach out to the team.
          </p>
          <a
